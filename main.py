...
class MyDataLoader:
    def __init__(self):
        ...
        self.plain_transform = T.Compose(
            [T.ToTensor(), T.Normalize(list(mean), list(std))]
        )

    def load_datasets(self):
        train_ds_plain = DataClass(
...
train_loader_plain1 = data.DataLoader(
    dataset=train_ds_plain, batch_size=batch_size, shuffle=True, drop_last=True
)
